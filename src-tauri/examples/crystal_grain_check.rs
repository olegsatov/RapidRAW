...
use rapidraw_lib::crystal_grain::{
    CrystalGrainOptions, apply_crystal_grain_rgb, render_crystal_grain_channel,
};

fn main() {
    let input = std::env::args().nth(1).expect(
        "usage: crystal_grain_check <in.png> [out.png] [filling] [size] [layers] [std] [mono]",
    );
    let output = std::env::args()
        .nth(2)
        .unwrap_or_else(|| "crystal_grain_out.png".to_string());
    let filling: f32 = std::env::args()
        .nth(3)
        .and_then(|s| s.parse().ok())
        .unwrap_or(0.25);
    let size: f32 = std::env::args()
        .nth(4)
        .and_then(|s| s.parse().ok())
        .unwrap_or(5.0);
    let layers: u32 = std::env::args()
        .nth(5)
        .and_then(|s| s.parse().ok())
        .unwrap_or(30);
    let std_dev: f32 = std::env::args()
        .nth(6)
        .and_then(|s| s.parse().ok())
        .unwrap_or(0.5);
