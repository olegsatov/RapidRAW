...
fn main() {
    let input = std::env::args().nth(1).expect("usage: film_grain_check <in.png> [out.png] [mu_r] [n_mc]");
    let output = std::env::args().nth(2).unwrap_or_else(|| "grain_check_out.png".to_string());
    let mu_r: f32 = std::env::args().nth(3).and_then(|s| s.parse().ok()).unwrap_or(0.1);
    let n_mc: u32 = std::env::args().nth(4).and_then(|s| s.parse().ok()).unwrap_or(100);
