...
- [x] **Step 1: Add new columns and tables to `SCHEMA_V1`**
...
- [x] **Step 2: Bump schema version and add migration**
...
- [x] **Step 3: Add helper to check/get file metadata columns**
...
- [x] **Step 4: Add helper to update metadata columns**
...
- [x] **Step 1: Create module skeleton and imports**
...
- [x] **Step 2: Implement `load_image_metadata`**
...
- [x] **Step 3: Implement `save_image_metadata`**
...
- [x] **Step 4: Implement field-level setters**
...
- [x] **Step 5: Implement history stub hooks**
...
- [x] **Step 6: Register module in `lib.rs`**

Add `mod metadata_store;` near other module declarations.

- [x] **Step 7: Verify cargo check**
...
- [x] **Step 1: Add `update_file_rating_flag_tags`**
...
- [x] **Step 1: Replace `sidecar_modified` fingerprint**
...
- [x] **Step 2: Update delta computation**
...
- [x] **Step 3: Remove `sidecar_modified` from `FileRowInput`**

Drop the `sidecar_modified` field and its usage in `upsert_files`.

- [x] **Step 4: Verify cargo check**
...
- [x] **Step 1: Update `save_metadata_and_update_thumbnail`**
...
- [x] **Step 2: Update `apply_adjustments_to_paths`**
...
- [x] **Step 1: Update `set_rating_for_paths`**
...
- [x] **Step 2: Update `set_flag_for_paths`**

Similarly use `metadata_store::set_flag`.

- [x] **Step 3: Update `set_color_label_for_paths`**

Use `metadata_store::set_color`.

- [x] **Step 4: Update tag editing in `tagging.rs`**
...
- [x] **Step 1: Update `persist_exif_if_missing`**
...
- [x] **Step 2: Update `write_rrexif_sidecar`**
...
- [x] **Step 3: Update `load_sidecar` legacy behavior**
...
- [x] **Step 4: Update `get_cache_key_hash` / `compute_thumbnail_cache_hash`**

Read adjustments JSON from DB (via `metadata_store::load_adjustments`) instead of sidecar.

- [x] **Step 5: Update export and film grain**
...
- [x] **Step 1: Update `create_virtual_copy`**
...
- [x] **Step 3: Remove `.rrdata` copy/rename/delete logic**
...
- [x] **Step 1: Check command signatures**
...
- [x] **Step 1: Add `metadata_store` unit tests**
...
- [x] **Step 2: Run full Rust test suite**
...
- [x] Schema covers `adjustments_json`, `metadata_modified`, `exif_json`, deltas, snapshots.
- [x] `metadata_store` is the single entry point for metadata read/write.
- [x] Legacy `.rrdata` fallback is lazy and read-only.
- [x] All write commands route through `metadata_store`.
- [x] All read commands route through `metadata_store`.
- [x] Delta sync uses `metadata_modified`.
- [x] XMP remains untouched as external sidecar.
- [x] History hooks are stubs ready for the next task.
